...
function ProcessKeyPress(e){
    ...
    if (e.keyCode == 13) {
            ...
            \"Type\":\"solution_list\",\
                ...
                {\
                    \"type\": \"local\",\
                        ...
                        \"summary\": \"this is the solution of.....\"\
                },\
            {\
                \"type\": \"google\",\
                    \"rating\": 8,\
        ...
        //for demo
        var testJson2 = $.parseJSON(testJson2);
...
function HandleSolutionResponse(response){
    $("div.OneSolution").each(function(){
        $(this).remove();
    });
    for(var index in response["Solutions"]){
        var one_solution = response["Solutions"][index];
        var solutionDom = $("div#hidden_solution").clone();
        solutionDom.attr("style","");
        solutionDom.attr("id","");
        solutionDom.removeClass("OneSolution_hidden");
        solutionDom.addClass("OneSolution");
        solutionDom.find("strong.SolutionRate").html(one_solution["rating"]);
        solutionDom.find("div.SolutionContent>h4").html(one_solution["title"]);
        solutionDom.find("div.SolutionContent>p").html(one_solution["summary"]);
        console.log(solutionDom.find("a.SolutionLink").attr("href"));
        solutionDom.find("a.SolutionLink").attr("href","www.configmaster.com/display_solution.html?"+one_solution["id"]);
        $("div.BottomSection.SearchedSolutions").append(solutionDom);
    }
